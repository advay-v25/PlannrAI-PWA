...
export function Step2Rhythm() {
    ...
    return (
        <div className="flex flex-col items-center justify-start space-y-8 w-full max-w-xl mx-auto pb-24">
            ...
            >
                {/* 🌙 SLEEP & WAKE */}
                <div className="space-y-6 bg-white/5 border border-white/10 p-7 rounded-3xl backdrop-blur-md shadow-lg">
                    <div className="flex items-center gap-3 text-lg font-bold text-white uppercase tracking-widest border-b border-white/10 pb-4">
                        <span className="text-2xl drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">🌙</span> SLEEP & WAKE
                    </div>
                    
                    <div className="grid grid-cols-2 gap-5">
                        <div className="space-y-3">
                            <label className="text-[10px] font-bold tracking-widest text-white/50 uppercase ml-1">Usually fall asleep</label>
                            <input
                                type="time"
                                value={data.sleep_start}
                                onChange={(e) => updateData({ sleep_start: e.target.value })}
                                className="w-full bg-black/40 border border-white/10 hover:border-white/30 rounded-2xl p-4 text-white focus:outline-none focus:border-white transition-all font-mono tracking-wider shadow-inner"
                            />
                        </div>
                        <div className="space-y-3">
                            <label className="text-[10px] font-bold tracking-widest text-white/50 uppercase ml-1">Usually wake up</label>
                            <input
                                type="time"
                    ...
                    <div className="space-y-3 pt-2">
                        <label className="text-[10px] font-bold tracking-widest text-white/50 uppercase ml-1">Wind-down time before sleep</label>
                ...
                <div className="space-y-6 bg-white/5 border border-white/10 p-7 rounded-3xl backdrop-blur-md shadow-lg">
                    ...
                    <div className="space-y-3">
                        <label className="text-[10px] font-bold tracking-widest text-white/50 uppercase ml-1">Meals per day</label>
                                ...
                                <button
                                    ...
                                    className={`py-5 rounded-3xl text-sm font-bold tracking-wide transition-all duration-300 flex-1 flex flex-col items-center justify-center gap-2 border ${
                                        data.meals_per_day === num 
                                            ...
                                            : 'bg-black/20 text-white/50 border-white/10 hover:bg-white/10 hover:text-white hover:scale-[1.01]'
                                    }`}
                                >
