    ...
    return (
        <div className="flex flex-col items-center justify-start space-y-8 w-full max-w-xl mx-auto pb-24">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="w-full space-y-10 mt-4">
                ...
                {/* Templates */}
                <div className="space-y-5 bg-white/5 border border-white/10 p-7 rounded-3xl backdrop-blur-md shadow-lg">
                    <p className="flex items-center gap-3 text-lg font-bold text-white uppercase tracking-widest border-b border-white/10 pb-4">
                    ...
                    <div className="grid grid-cols-2 gap-5">
                        {TEMPLATES.map((t, i) => (
                            <button
                                ...
                                className="p-6 rounded-2xl bg-black/20 border border-white/10 hover:bg-white/10 hover:scale-[1.03] hover:border-white/30 text-left transition-all duration-300 group shadow-inner"
                            >
                                <div className="font-bold text-lg text-white tracking-wide group-hover:text-[var(--color-primary)] transition-colors drop-shadow-md">{t.title}</div>
                                <div className="text-xs text-white/50 mt-2 font-mono tracking-wider flex items-center gap-1.5 opacity-80">
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                        <h3 className="text-sm font-bold text-white/80 tracking-widest uppercase">Your Commitments</h3>
                    ...
                    <div className="space-y-4">
                        <AnimatePresence>
                            {commitments.length === 0 && !isAdding && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center p-10 border-2 border-dashed border-white/10 rounded-3xl text-white/40 text-sm font-medium tracking-wide bg-white/[0.02]">
                            ...
                            {commitments.map((c, i) => (
                                <motion.div
                                    ...
                                    className="bg-white/5 border border-white/10 p-6 rounded-3xl flex items-center justify-between group overflow-hidden backdrop-blur-md hover:bg-white/[0.08] hover:border-white/30 transition-all shadow-lg"
                                >
                                    <div className="space-y-2">
                                        <div className="font-black text-white text-xl tracking-tight drop-shadow-md">{c.title}</div>
                                        ...
                                        <div className="text-xs text-white/50 font-mono tracking-widest flex items-center gap-1.5">
                                            <Clock className="w-3.5 h-3.5 opacity-70" /> {c.start_time} - {c.end_time}
                                        </div>
                                    </div>
                                    <button onClick={() => removeCommitment(i)} className="p-3.5 text-white/30 hover:text-red-400 hover:bg-red-400/20 rounded-full transition-all bg-black/20 hover:scale-110 active:scale-95 border border-transparent hover:border-red-400/30">
                    ...
                    <AnimatePresence>
                        {isAdding ? (
                            <motion.div
                                ...
                                className="bg-black/60 border border-[var(--color-primary)]/30 p-7 rounded-3xl space-y-6 overflow-hidden backdrop-blur-xl shadow-[0_0_30px_rgba(var(--color-primary-rgb),0.15)] mt-4"
                            >
                                <div className="space-y-4">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white text-lg focus:outline-none focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] transition-all font-bold placeholder:text-white/20 placeholder:font-normal"
                                        autoFocus
                                    />
                                </div>
                                <div className="grid grid-cols-2 gap-5">
                                    <div className="space-y-3 relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white focus:outline-none focus:border-[var(--color-primary)] transition-all font-mono tracking-wider text-base"
                                        />
                                    </div>
                                    <div className="space-y-3 relative">
                                ...
                                <div className="flex gap-4 pt-6">
                                    <button onClick={() => setIsAdding(false)} className="flex-[0.4] py-4 text-xs font-bold uppercase tracking-widest text-white/50 hover:text-white border border-white/10 rounded-2xl hover:bg-white/5 hover:border-white/30 transition-all bg-black/20">
                                        Cancel
                                    </button>
                            ...
                            <button onClick={() => setIsAdding(true)} className="w-full py-8 mt-4 border border-dashed border-white/20 hover:border-[var(--color-primary)] hover:bg-[var(--color-primary)]/[0.03] rounded-3xl flex items-center justify-center gap-3 text-white/40 hover:text-white transition-all duration-300 font-bold text-sm group tracking-widest uppercase">
