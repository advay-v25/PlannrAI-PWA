...
export function Step3Anchors() {
    ...
    const [editComm, setEditComm] = useState<Partial<OnboardingCommitment>>({});
    ...
    const startEditing = (index: number, comm: OnboardingCommitment) => {
        setEditingIndex(index);
        setEditComm({ ...comm });
    };

    const toggleEditDay = (dayId: number) => {
        const days = editComm.days_of_week || [];
        if (days.includes(dayId)) {
            setEditComm({ ...editComm, days_of_week: days.filter(d => d !== dayId) });
        } else {
            setEditComm({ ...editComm, days_of_week: [...days, dayId].sort() });
        }
    };

    const handleSaveEdit = (index: number) => {
        if (!editComm.title || !editComm.start_time || !editComm.end_time || !editComm.days_of_week?.length) return;
        const next = [...commitments];
        next[index] = editComm as OnboardingCommitment;
        updateData({ commitments: next });
                                    ...
                                    {editingIndex === i ? (
                                        <div className="w-full space-y-4">
                                            <div className="space-y-2 text-left">
                                                <label className="text-[9px] font-bold text-[var(--color-primary)] uppercase tracking-widest pl-1">Edit Title</label>
                                                <input
                                                    type="text"
                                                    value={editComm.title}
                                                    onChange={e => setEditComm({ ...editComm, title: e.target.value })}
                                                    className="w-full bg-[var(--glass-bg-active)] border border-[var(--glass-border)] p-3.5 rounded-xl text-[var(--text-primary)] text-base focus:outline-none focus:border-[var(--color-primary)] transition-all font-bold placeholder:text-[var(--text-primary)]/20"
                                                />
                                            </div>
                                            
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-2 text-left">
                                                    <label className="text-[9px] font-bold text-[var(--color-primary)] uppercase tracking-widest pl-1">Start Time</label>
                                                    <input
                                                        type="time"
                                                        value={editComm.start_time}
                                                        onChange={e => setEditComm({ ...editComm, start_time: e.target.value })}
                                                        className="w-full bg-[var(--glass-bg-active)] border border-[var(--glass-border)] p-3 rounded-xl text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-primary)] transition-all font-mono text-sm"
                                                    />
                                                </div>
                                                <div className="space-y-2 text-left">
                                                    <label className="text-[9px] font-bold text-[var(--color-primary)] uppercase tracking-widest pl-1">End Time</label>
                                                    <input
                                                        type="time"
                                                        value={editComm.end_time}
                                                        onChange={e => setEditComm({ ...editComm, end_time: e.target.value })}
                                                        className="w-full bg-[var(--glass-bg-active)] border border-[var(--glass-border)] p-3 rounded-xl text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-primary)] transition-all font-mono text-sm"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-2 pt-1 text-left">
                                                <label className="text-[9px] font-bold text-[var(--color-primary)] uppercase tracking-widest pl-1">Active Days</label>
                                            ...
                                            <div className="flex gap-3 pt-2">
                                                <button
                                                    ...
                                                    className="flex-[0.4] py-2 rounded-xl border border-[var(--glass-border)] text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/50 hover:bg-[var(--glass-bg-active)] transition-colors duration-300"
                                                >
                                                    Cancel
                                                </button>
                                                <button
                                        ...
                                        <div className="flex items-center justify-between w-full">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button 
                                                    ...
                                                    className="p-3 text-[var(--text-primary)]/30 hover:text-red-400 hover:bg-red-400/20 rounded-full transition-all bg-[var(--glass-bg-active)] hover:scale-105 active:scale-95 border border-transparent hover:border-red-400/30"
                                                >
