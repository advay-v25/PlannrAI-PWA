...
export default function HistoryDatePage() {
    ...
    if (!hasData || !isValidDate) {
        return (
            <div className="flex-1 h-full flex flex-col relative overflow-hidden">
                ...
                <header className="h-16 flex items-center px-6 border-b border-[var(--glass-border)] bg-black/20 backdrop-blur-md z-10">
                    <button 
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 text-center z-10">
                    ...
                    <p className="text-[var(--text-secondary)] max-w-sm mx-auto mb-8 text-sm">
                        We couldn't find any historical data or schedule blocks for <span className="text-white font-medium">{displayDate}</span>. The daily log may be empty or the date is out of bounds.
                    </p>

                    <button 
                        onClick={() => router.push('/app')}
                        className="px-6 py-3 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white font-bold rounded-xl transition-all shadow-[0_0_15px_var(--color-primary-glow)] flex items-center gap-2"
                    >
    ...
    return (
        <div className="flex-1 h-full flex flex-col items-center justify-center p-6 text-center">
            <PageBackground variant="aurora" intensity="strong" />
            <h1 className="text-2xl font-bold text-white mb-4">Historical Data for {displayDate}</h1>
