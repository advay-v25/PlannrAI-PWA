    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-[var(--text-primary)] tracking-tight">Commitments</h2>
                    <p className="text-sm text-[var(--text-tertiary)] mt-1">Locked blocks that AI cannot move</p>
                </div>
                {!isAdding && (
                    <button
            ...
            <AnimatePresence>
                {isAdding && (
                    <motion.div
                        ...
                        className="rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg)] p-5 space-y-4"
                    >
                        <div className="flex items-center justify-between">
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2.5 text-sm bg-[var(--color-bg-primary)] border border-[var(--glass-border)] rounded-xl
                                text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none focus:border-[var(--color-primary)]/30 transition-all"
                        />
                        <div className="flex gap-3">
                            <div className="flex-1 space-y-1">
                                <label className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider font-bold">Start</label>
                                <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)}
                                    className="w-full px-3 py-2 text-sm bg-[var(--color-bg-primary)] border border-[var(--glass-border)] rounded-lg text-[var(--text-primary)] outline-none" />
                            </div>
                            <div className="flex-1 space-y-1">
                                <label className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider font-bold">End</label>
                                <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)}
                                    className="w-full px-3 py-2 text-sm bg-[var(--color-bg-primary)] border border-[var(--glass-border)] rounded-lg text-[var(--text-primary)] outline-none" />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider font-bold">Days</label>
                                ...
                                {DAYS.map(day => (
                                    <button
                                        ...
                                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${days.includes(day)
                                                ? 'bg-[var(--color-primary)] text-[var(--text-primary-inverse,white)]'
                                                : 'bg-[var(--color-bg-primary)] border border-[var(--glass-border)] text-[var(--text-tertiary)] hover:bg-[var(--glass-bg-hover)]'
                                            }`}
                                    >
                                        {day}
                                    </button>
            ...
            <div className="space-y-3">
                {commitments.length === 0 && !isAdding && (
                    <div className="text-center py-12 border border-[var(--glass-border)] rounded-2xl bg-[var(--glass-bg)]">
                        <Lock className="w-10 h-10 text-[var(--text-muted)] mx-auto mb-3" />
                        <p className="text-sm text-[var(--text-tertiary)]">No commitments yet</p>
                        <p className="text-xs text-[var(--text-muted)] mt-1">Add blocks that AI should never move</p>
                    </div>
                )}
                {commitments.map(c => (
                    <motion.div
                        ...
                        <div className="flex items-start justify-between">
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <h4 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-wide">{c.title}</h4>
                                </div>
                            ...
                            <div className="flex items-center gap-1.5">
                                <button
                                    onClick={() => startEdit(c)}
                                    className="p-1.5 rounded-lg text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--glass-bg-hover)] transition-all"
                                >
