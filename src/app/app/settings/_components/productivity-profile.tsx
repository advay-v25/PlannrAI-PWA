...
export default function ProductivityProfile() {
    ...
    if (!profile || profile.data_points < 5) {
        return (
            <div className="space-y-4">
                <div>
                    <h2 className="text-xl font-bold text-white tracking-tight">Productivity Profile</h2>
                    <p className="text-sm text-white/40 mt-1">AI-generated from your patterns</p>
                </div>
                <div className="text-center py-12 border border-white/5 rounded-2xl bg-white/[0.02]">
                    ...
                    <p className="text-xs text-white/20 mt-1">Complete more blocks to unlock your productivity profile</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-xl font-bold text-white tracking-tight">Productivity Profile</h2>
                <p className="text-sm text-white/40 mt-1">AI-generated from {profile.data_points} data points</p>
            </div>
            ...
            >
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-lg font-bold text-white">{profile.archetype}</h3>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                    ...
                    <div className="text-lg font-bold text-white">{profile.peak_window.start} - {profile.peak_window.end}</div>
                    <div className="text-xs text-white/40 mt-0.5">{profile.peak_window.completion_rate}% completion rate</div>
                </div>
                <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                    ...
                    <div className="text-lg font-bold text-white">{profile.low_window.start} - {profile.low_window.end}</div>
                    <div className="text-xs text-white/40 mt-0.5">{profile.low_window.completion_rate}% completion rate</div>
                </div>
            </div>

            {/* Day Patterns */}
            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5 space-y-3">
                <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider">Day Patterns</h4>
                <div className="space-y-2">
                    {profile.best_days.map(d => (
                        <div key={d.day} className="flex items-center justify-between">
                            <span className="text-sm text-white/70 capitalize">{d.day}</span>
                    ...
                    {profile.worst_day && (
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-white/70 capitalize">{profile.worst_day.day}</span>
                            <div className="flex items-center gap-2">
                                <div className="w-32 h-2 rounded-full bg-white/5 overflow-hidden">
            ...
            {profile.pillar_insights.length > 0 && (
                <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5 space-y-3">
                    <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider">Pillar Performance</h4>
                    <div className="grid grid-cols-3 gap-3">
                        {profile.pillar_insights.map(p => (
                            <div key={p.pillar} className="text-center">
                                ...
                                <div className="text-[10px] text-white/30 uppercase tracking-wider mt-0.5">{p.pillar}</div>
                            </div>
                        ))}
                    </div>
                </div>
