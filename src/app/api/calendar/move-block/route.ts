...
export const POST = secureApiRoute(
    async (context: any, body: any) => {
        const { blockId, newDate, newStart, newEnd } = body;

        if (!blockId || !newDate || !newStart || !newEnd) {
        ...
        try {
            const result = await CalendarEngine.moveBlock(context.user.id, blockId, newDate, newStart, newEnd, supabase);
