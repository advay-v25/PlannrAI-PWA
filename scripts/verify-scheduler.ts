...
const mockContext: any = {
    ...
    profile: {
        ...
        // Mocking required profile fields to satisfy types partially or expecting loose typing
        ai_can_analyze: true
    },
    commitments: [
...
(async () => {
    try {
        ...
        const blocks = await service.generateBaseline();

        console.log(`✅ Generated ${blocks.length} blocks for 3 days.`);

        // Basic assertions
        const sleepBlocks = blocks.filter((b: any) => b.title === 'Sleep');
        // Note: Sleep logic might not be in baseline anymore if moved to applyBasicConstraints in PlanWeekService?
        ...
        if (blocks.length > 0) {
            ...
            blocks
                .filter((b: any) => b.date === formattedDate)
